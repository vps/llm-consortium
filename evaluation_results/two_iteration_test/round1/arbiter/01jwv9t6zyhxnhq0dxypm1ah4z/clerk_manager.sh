...
_run_clerk_interaction() {
    ...
    local conv_exists=$(sqlite3 -batch -noheader "$db_path" "SELECT COUNT(*) FROM responses WHERE conversation_id='$effective_cid';" 2>/dev/null || echo "0"); conv_exists="${conv_exists:-0}"
...
vibelab_complete_task() {
    ...
    if [[ "$task_id_or_keywords" =~ ^[0-9a-zA-Z]{26}$ ]]; then
        task_response_id_check=$(sqlite3 -batch -noheader "$db_path" "SELECT id FROM responses WHERE id='$task_id_or_keywords' AND conversation_id='$pending_cid' LIMIT 1;")
    ...
    if [ -z "$task_response_id" ]; then
        ...
        task_response_id=$(sqlite3 -batch -noheader "$db_path" "SELECT id FROM responses WHERE conversation_id='$pending_cid' AND (prompt LIKE '%$task_id_or_keywords%' OR response LIKE '%$task_id_or_keywords%') ORDER BY datetime_utc DESC LIMIT 1;")
        if [ -z "$task_response_id" ]; then
    ...
    sqlite3 -batch -noheader "$db_path" "UPDATE responses SET conversation_id='${completed_cid}' WHERE id='${task_response_id}';"
    
    if [ $? -eq 0 ]; then
        echo "Task (ID: $task_response_id) moved from VibeLab pending ($pending_cid) to completed ($completed_cid) context."
        local task_prompt_content=$(sqlite3 -batch -noheader "$db_path" "SELECT prompt FROM responses WHERE id='${task_response_id}' LIMIT 1;")
